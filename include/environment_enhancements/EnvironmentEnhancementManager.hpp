...
struct ProfileData {
  ...
  ProfileData(std::chrono::high_resolution_clock::time_point const& time, std::string_view log, bool showTime)
      : time(time), log(log), showTime(showTime) {}

  ProfileData(std::chrono::high_resolution_clock::time_point const& time, std::string_view log)
      : time(time), log(log) {}
};
