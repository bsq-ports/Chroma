...
custom_types::Helpers::Coroutine Chroma::EnvironmentMaterialManager::Activate() {
    ...
    auto Load = [](std::string_view environmentName) {
        ...
        return UnityEngine::SceneManagement::SceneManager::LoadSceneAsync(environmentName,
                                                                          UnityEngine::SceneManagement::LoadSceneMode::Additive);
    };

    auto environments = std::array<std::string_view, 3>(
            {"BTSEnvironment", "BillieEnvironment", "InterscopeEnvironment"});
    auto loads = environments | Select([&](std::string_view s) {
        return Load(s);
    });
    ...
    auto Save = [&](ShaderType key, std::string_view matName) {
        auto material = environmentMaterials.FirstOrDefault(
                [&](auto const &e) { return e->get_name() == matName; });
        if (material != nullptr) {
