...
MAKE_HOOK_MATCH(NoteController_Init, &NoteController::Init, void,
                ...
                float uniformScale, bool rotateTowardsPlayer,
  ...
  NoteController_Init(self, noteData, worldRotation, moveStartPos, moveEndPos,
                      ...
                      endRotation, uniformScale, rotateTowardsPlayer,
                      useRandomRotation);
...
MAKE_HOOK_MATCH(NoteController_Update, &NoteController::ManualUpdate, void,
                NoteController *self) {
  ...
  if (chromaData != ChromaObjectDataManager::ChromaObjectDatas.end()) {
    ...
    if (!tracks.empty() || pathPointDefinition) {
      ...
      float elapsedTime = ChromaTimeSourceHelper::getSongTimeChroma(
                              noteJump->audioTimeSyncController) -
